...
long smarterSum(long n) {
    long total = 1; // 1 is a divisor for all numbers
    long sqrt_num = (long) sqrt(n);

    // Start from 2 as we have already considered 1
    for (long divisor = 2; divisor <= sqrt_num; divisor++) {
        if (n % divisor == 0) {
            // If divisor and corresponding divisor are the same, add it only once
...
STUDENT_TEST("If smartSum act same as divisorSum") {
    EXPECT_EQUAL(divisorSum(2), smarterSum(2));
    EXPECT_EQUAL(divisorSum(6), smarterSum(6));
    EXPECT_EQUAL(divisorSum(12), smarterSum(12));
    EXPECT_EQUAL(divisorSum(20), smarterSum(20));
    EXPECT_EQUAL(divisorSum(50), smarterSum(50));
    EXPECT_EQUAL(divisorSum(100), smarterSum(100));
}

// STUDENT_TEST("FindNthPerfectEuclid function") {
//     EXPECT(isPerfect(findNthPerfectEuclid(2)));
//     EXPECT(isPerfect(findNthPerfectEuclid(3)));
//     EXPECT(isPerfect(findNthPerfectEuclid(5)));
//     EXPECT(isPerfect(findNthPerfectEuclid(7)));
//     EXPECT(isPerfect(findNthPerfectEuclid(8)));
//     EXPECT(isPerfect(findNthPerfectEuclid(10)));
//     EXPECT(isPerfect(findNthPerfectEuclid(12)));
//     EXPECT(isPerfect(findNthPerfectEuclid(231)));
//     EXPECT(isPerfect(findNthPerfectEuclid(13)));
//     EXPECT(isPerfect(findNthPerfectEuclid(14)));
//     EXPECT(isPerfect(findNthPerfectEuclid(50)));
// }
